...
const map = (g, ls) => {
  return fold_left((acc, x) => acc.push(g(x)), List([]), ls);
};
...
const filter = (g, ls) => {
  ...
  return fold_left(foldFn, List([]), ls);
};
...
const partition = (g, ls) => {
  return List([filter(g, ls), filter(x => !g(x), ls)]);
};
...
const quicksort = function(ls) {
  ...
  const others = ls.shift();
  
  const [less, greater] = partition(x => x <= pivot, others);
  
  return quicksort(less).concat(List([pivot])).concat(quicksort(greater));
};
